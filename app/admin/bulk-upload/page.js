...
export default function BulkUploadPage() {
  ...
  const uploadFileAndCreateDraft = async (uploadItem) => {
    try {
      ...
      if (!result.ok) {
        throw new Error(await result.text() || "Failed to upload file to Convex");
      }
    ...
    <div>
      <div style={{ marginBottom: '2.5rem' }}>
        ...
        <div style={{ marginTop: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Assign to Category:</label>
          <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} style={{ padding: '0.6rem 1rem', background: '#0c0c14', border: '1px solid var(--border-color)', borderRadius: '8px', color: '#fff', outline: 'none', width: '250px' }}>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '24px', padding: '2rem', height: '100%', minHeight: '400px' }}>
            ...
            {uploads.length > 0 && uploads.every(u => u.status === 'complete' || u.status === 'error') && (
              <div style={{ marginTop: '2rem', textAlign: 'center' }}>
                <Link href="/admin" className="btn" style={{ background: 'var(--primary)', color: '#fff', padding: '0.75rem 2rem', borderRadius: '100px', fontWeight: 700, fontSize: '1rem', boxShadow: '0 0 20px var(--primary-glow)', animation: 'pulseGlow 2s infinite' }}>
                  Return to Dashboard
                </Link>
              </div>
            )}
      ...
      <style jsx>{`
        ...
        @keyframes pulseGlow {
          ...
          50% { box-shadow: 0 0 25px rgba(181, 53, 246, 0.6); }
