...
export async function sendServiceConfirmationEmail(serviceData, baseUrl) {
  const adminEmails = ['mohammadsayemweb@gmail.com', 'htmlocean@gmail.com'];
  
  const isPaid = serviceData.price_usd && serviceData.price_usd > 0;
  
  const confirmText = isPaid 
    ? '<p style="color: #10b981; font-weight: 800; font-size: 12px; text-transform: uppercase; letter-spacing: 2px; margin: 0;">Payment Verified</p>'
    : '<p style="color: #8b5cf6; font-weight: 800; font-size: 12px; text-transform: uppercase; letter-spacing: 2px; margin: 0;">Request Received</p>';
    
  const bodyText = isPaid
    ? '<p style="font-size: 15px; color: #c5c7d0; line-height: 1.7; margin: 0;">Your payment for <strong>' + serviceData.service_type + '</strong> was successfully verified on the blockchain. Our expert team will review your requirements and reach out to you shortly.</p>'
    : '<p style="font-size: 15px; color: #c5c7d0; line-height: 1.7; margin: 0;">We have received your custom request for <strong>' + serviceData.service_type + '</strong>. Our expert team will review your requirements and reach out to you shortly.</p>';
  ...
  const htmlContent = `
    <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #07070f; color: #e8e9f0; border-radius: 16px; overflow: hidden; border: 1px solid rgba(255,255,255,0.06);">
      ...
      <div style="background: linear-gradient(135deg, #1a0a2e 0%, #0d0d1e 100%); padding: 40px; border-bottom: 1px solid rgba(139,92,246,0.2); text-align: center;">
        ...
        ${confirmText}
        <h1 style="margin: 12px 0 0; font-size: 28px; font-weight: 900; color: #fcfcfd; letter-spacing: -0.5px;">Service Confirmed</h1>
      </div>

      <!-- Body -->
      <div style="padding: 40px;">
        <p style="font-size: 16px; font-weight: 600; color: #fcfcfd; margin: 0 0 12px;">Hi ${serviceData.name ? serviceData.name.split(' ')[0] : 'Customer'},</p>
        ${bodyText}

        <!-- Request Details -->
        <div style="margin-top: 32px; background: rgba(139,92,246,0.04); border: 1px solid rgba(139,92,246,0.15); border-radius: 12px; padding: 24px;">
          <h2 style="margin: 0 0 16px; font-size: 11px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; color: #8b5cf6;">Project Details</h2>
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 6px 0; color: #6b6f7b; font-size: 14px;">Service Type</td>
              <td style="padding: 6px 0; text-align: right; color: #c5c7d0; font-size: 14px; font-weight: 600;">${serviceData.service_type}</td>
            </tr>
            <tr>
              <td style="padding: 6px 0; color: #6b6f7b; font-size: 14px;">Budget</td>
              <td style="padding: 6px 0; text-align: right; color: #c5c7d0; font-size: 14px; font-weight: 600;">${serviceData.budget || 'Not specified'}</td>
            </tr>
            ${isPaid ? `
            <tr>
              <td style="padding: 16px 0 0; border-top: 1px solid rgba(255,255,255,0.06); color: #fcfcfd; font-size: 16px; font-weight: 700;">Total Paid</td>
              <td style="padding: 16px 0 0; border-top: 1px solid rgba(255,255,255,0.06); text-align: right; color: #10b981; font-size: 18px; font-weight: 800;">$${serviceData.price_usd.toFixed(2)} USD</td>
            </tr>
            ` : ''}
          </table>
          <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.06);">
            <h3 style="margin: 0 0 8px; font-size: 12px; color: #6b6f7b; text-transform: uppercase;">Your Message</h3>
            <p style="margin: 0; font-size: 14px; color: #a1a1aa; line-height: 1.6;">${serviceData.message}</p>
          </div>
        </div>
      </div>
      ...
      <div style="padding: 32px 40px; border-top: 1px solid rgba(255,255,255,0.04); text-align: center; background: rgba(0,0,0,0.2);">
        <p style="margin: 0 0 12px; font-size: 13px; color: #6b6f7b; line-height: 1.6;">
          Our support team will reach out to <strong>${serviceData.email}</strong>.<br>
          If you have immediate questions, feel free to reply to this email.
        </p>
        <p style="margin: 0; font-size: 12px; color: #4b4f5b;">Themes Zoo · Premium Digital Assets & Services</p>
      </div>
    </div>
  `;
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        bcc: adminEmails,
        subject: `Confirmed: Custom Service Request - ${serviceData.service_type}`,
        html: htmlContent,
      })
    });
...
export async function sendTicketNotificationEmail(ticketData) {
  const adminEmails = ['mohammadsayemweb@gmail.com', 'htmlocean@gmail.com'];
  ...
  const htmlContent = `
    <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background: #07070f; color: #e8e9f0; padding: 40px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.06);">
      <h1 style="color: #8b5cf6;">Support Ticket Received</h1>
      <p>Hi ${ticketData.name},</p>
      <p>We have received your support ticket regarding <strong>${ticketData.subject}</strong>.</p>
      <p>Our support team has been notified and will reply to this email shortly.</p>
      <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 8px; margin-top: 20px;">
        <p><strong>Your Message:</strong><br>${ticketData.message}</p>
      </div>
    </div>
  `;
