...
export async function sendServiceConfirmationEmail(serviceData, baseUrl) {
  ...
  const htmlContent = `
    <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background: #07070f; color: #e8e9f0; padding: 40px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.06);">
      <h1 style="color: #10b981;">Service Request Confirmed</h1>
      <p>Hi ${serviceData.name || 'Customer'},</p>
      <p>We have received your payment/request for: <strong>${serviceData.service_type}</strong>.</p>
      <p>Our team will review your requirements and reach out to you shortly at ${serviceData.email}.</p>
      <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 8px; margin-top: 20px;">
        <p><strong>Message/Requirements:</strong><br>${serviceData.message}</p>
      </div>
    </div>
  `;
