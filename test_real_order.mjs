import { ConvexHttpClient } from "convex/browser";
import { Resend } from "resend";

const convex = new ConvexHttpClient("https://frugal-cobra-312.convex.cloud");
const resend = new Resend("re_b6oKN2cQ_G7uiFiwGGu1h65Bk2A4KX6zs");

async function testFlow() {
  try {
    // 1. Get a real product
    const products = await convex.query("products:listAll", {});
    const product = products.find(p => p.file_id || p.file_url);
    
    if (!product) {
      console.error("No product found with a file attached. Please upload a file to a product first.");
      return;
    }

    // 2. Create a paid order
    const orderId = await convex.mutation("orders:create", {
      product_id: product._id,
      product_name: product.name,
      price_usd: product.price_usd,
      buyer_email: "mohammadsayemweb@gmail.com",
      buyer_name: "Mohammad Sayem",
      buyer_id: "test_buyer_id",
      status: "paid",
      tx_hash: "mock_tx_hash_for_test",
    });

    console.log("Created Mock Paid Order ID:", orderId);

    // 3. Send the email
    const buyerEmail = "mohammadsayemweb@gmail.com";
    const baseUrl = "https://theme-zoo.vercel.app"; // Using their probable Vercel domain or localhost

    const itemsHtml = `
      <div style="padding: 20px; margin-bottom: 16px; background-color: #121214; border: 1px solid #27272a; border-radius: 12px;">
        <h3 style="margin: 0 0 6px 0; color: #f4f4f5; font-size: 18px; font-weight: 600;">${product.name}</h3>
        <p style="margin: 0 0 16px 0; color: #a1a1aa; font-size: 14px;">Price: $${product.price_usd.toFixed(2)} USD</p>
        <a href="${baseUrl}/download/${orderId}" style="display: inline-block; padding: 12px 24px; background-color: #7c3aed; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 14px;">Download Asset</a>
      </div>
    `;

    const htmlContent = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #09090b; padding: 40px 24px; color: #ededed; border: 1px solid #18181b; border-radius: 16px;">
        <div style="text-align: center; margin-bottom: 32px;">
          <div style="display: inline-flex; justify-content: center; align-items: center; width: 48px; height: 48px; border-radius: 50%; background-color: rgba(16, 189, 129, 0.1); margin-bottom: 16px;">
            <span style="font-size: 24px; color: #10b981;">✓</span>
          </div>
          <h1 style="color: #ffffff; font-size: 24px; font-weight: 800; margin: 0 0 8px 0; letter-spacing: -0.5px;">Payment Confirmed</h1>
          <p style="color: #10b981; font-weight: 600; font-size: 14px; margin: 0;">Crypto Transaction Verified</p>
        </div>
        
        <p style="font-size: 15px; line-height: 1.6; color: #d4d4d8; margin-bottom: 24px;">Hi Mohammad Sayem,</p>
        <p style="font-size: 15px; line-height: 1.6; color: #d4d4d8;">Thank you for your purchase. Your crypto payment equivalent to <strong>$${product.price_usd.toFixed(2)} USD</strong> has been successfully verified on the blockchain. Your source files are now unlocked and ready for download.</p>
        
        <div style="margin: 40px 0;">
          <h2 style="font-size: 12px; text-transform: uppercase; letter-spacing: 1.5px; color: #71717a; border-bottom: 1px solid #27272a; padding-bottom: 12px; margin-bottom: 20px;">Your Authorized Downloads</h2>
          ${itemsHtml}
        </div>
        
        <p style="font-size: 13px; line-height: 1.5; color: #71717a; text-align: center; margin-top: 40px; padding-top: 24px; border-top: 1px solid #27272a;">
          These links are tied to your email (${buyerEmail}) and will remain active for 48 hours. Please securely backup your files.<br><br>
          If you need any assistance, simply reply to this email.
        </p>
      </div>
    `;

    const data = await resend.emails.send({
      from: "ThemesZoo <onboarding@resend.dev>",
      to: buyerEmail,
      subject: \`Your Receipt & Download Links - $\${product.price_usd.toFixed(2)}\`,
      html: htmlContent,
    });

    console.log("Email sent successfully:", data);

  } catch (err) {
    console.error("Error:", err);
  }
}

testFlow();
